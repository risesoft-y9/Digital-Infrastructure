...
function softDowLoad(i) {
    if (i == 0) {
        if (isInWeb == false) {
            ...
            window.open("http://10.1.4.123/Soft/Index.asp")
        }
    } else if (i == 1) {
        if (isInWeb == false) {
            alert("病毒防范仅限内网使用，请登录内网机访问。");
            return false;
        } else {
            window.open("http://10.1.3.23/virus/index.asp");
        }
    }
}
...
function getTopMen(iweb) {
    ...
    if (isInWeb == false)//外网
    {
        ...
        emailstr = '<li><a href="http://10.1.4.123/dd.htm">公司邮箱</a></li><li><a href="http://mail.crc.cr/indishare/office.nsf/(frame)/mail">铁总邮箱</a></li>';
    }
    ;
    switch (iweb) {
        case 0:
            ...
            break;
        case 1:
            logo = "/sso/y9static/images/zhongtie/logo/logo_jsfw.png;" + '\"style="width:580px;"\"'//技术服务
            break;
        case 2:
            logo = "/sso/y9static/images/zhongtie/logo/logo_dsjgstj.png";//大数据
            break;
        case 3:
            logo = "/sso/y9static/images/zhongtie/logo/logo_ztx.png";//中铁信
            break;
        case 4:
            logo = "/sso/y9static/images/zhongtie/logo/logo_jsjgs.png";//计算机
            break;
        case 5:
            logo = "/sso/y9static/images/zhongtie/logo/logo_sys.png";//城市
            break;
        case 6:
            logo = "/sso/y9static/images/zhongtie/logo/logo_hxsj.png";//弘信
            break;
        case 7:
            logo = "/sso/y9static/images/zhongtie/logo/logo_hyrj.png";//弘远
            break;
    }
...
function friendLists() {
    var flist = [{title: "中国铁路总公司", href: "http://www.crc.cr/", isWeb: false},
        ...
        {
            title: "上海铁路局",
            href: "http://10.128.4.75/lyj_lj/index/index.asp?unit_numb=1163&CurrDeptNumb=1163",
            isWeb: true
        },
    ...
    var fstr = '', pstr = '';
    for (var i = 0; i < flist.length; i++) {
        if ((isInWeb == true && flist[i].isWeb == true) || (isInWeb == false && flist[i].isWeb == false)) {
            fstr += '<a href="' + flist[i].href + '" target="_blank" >' + flist[i].title + '</a>';
        } else {
            fstr += '<a>' + flist[i].title + '</a>';
        }
    }
...
function HTMLLoad(iweb) {
    /*加载头部内容*/
    if (document.getElementById("topContent") != null) {
        document.getElementById("topContent").innerHTML = getTopMen(iweb);
    }
    getOldSyttem();
    //if (iweb == 0) {getOldSyttem();}
    /*其他家公司*/
    if (document.getElementById("divOtherCompany") != null) {
        document.getElementById("divOtherCompany").innerHTML = OtherCompany();
    }
    /*友情链接*/
    if (document.getElementById("divFriendlist") != null) {
        document.getElementById("divFriendlist").innerHTML = friendLists();
    }
    /*底部*/
    if (document.getElementById("divBottom") != null) {
        document.getElementById("divBottom").innerHTML = getBottom();
    }
}
