...
public class Y9logUserLoginInfoServiceImpl implements Y9logUserLoginInfoService {
	...
	public List<Object[]> listDistinctUserHostIpByUserIdAndLoginTime(String userId, Date startTime, Date endTime) {
		...
		SearchRequest searchRequest = SearchRequest.of(s -> s
			.index(Y9ESIndexConst.LOGIN_INFO_INDEX)
			.query(q -> q
				.bool(b -> b
					.must(m -> m
						.queryString(qs -> qs.fields(Y9LogSearchConsts.USER_ID).query(userId)))
					.must(m -> m
						.range(r -> r.field(Y9LogSearchConsts.LOGIN_TIME)
							.from(String.valueOf(startTime.getTime()))
							.to(String.valueOf(endTime.getTime()))
						)
					)
				)
			)
			.aggregations("by_userHostIp", a -> a
				.terms(t -> t.field(Y9LogSearchConsts.USER_HOST_IP))
			)
		);
		
		try {
			elasticsearchClient.search(searchRequest, Y9logUserLoginInfo.class)
				.aggregations().get("by_userHostIp").sterms().buckets().array()
					.forEach(bucket -> {
	...
	public List<Map<String, Object>> listUserHostIpByCip(String cip) {
		...
		SearchRequest searchRequest = SearchRequest.of(s -> s
			...
			.query(q -> q
				.bool(b -> b
					.must(m -> m
						.queryString(qs -> qs.fields(Y9LogSearchConsts.USER_HOST_IP).query(cip + "*"))
					)
				)
			)
			.aggregations("by_UserHostIP", a -> a
		...
		try {
			SearchResponse<Y9logUserLoginInfo> response = elasticsearchClient.search(searchRequest, Y9logUserLoginInfo.class);
			response.aggregations().get("by_UserHostIP")
	...
	public Y9Page<Y9logUserLoginInfo> page(String userHostIp, String userId, String success, String startTime,
        ...
        if (StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)) {
            ...
            try {
                criteria.and(Y9LogSearchConsts.LOGIN_TIME)
        ...
        SearchHits<Y9logUserLoginInfo> searchHits = elasticsearchTemplate.search(query, Y9logUserLoginInfo.class, INDEX);
	...
	public Page<Y9logUserLoginInfo> pageBySuccessAndServerIpAndUserName(String success, String userHostIp, 
        ...
        Pageable pageable = PageRequest
        	.of((page < 1) ? 0 : page - 1, rows, Sort.by(Sort.Direction.DESC, Y9LogSearchConsts.LOGIN_TIME));
	...
	@Override
	public Page<Y9logUserLoginInfo> pageByTenantIdAndManagerLevel(String tenantId, String managerLevel,
			int page, int rows) {
		Pageable pageable = PageRequest
			.of((page < 1) ? 0 : page - 1, rows, Sort.by(Sort.Direction.DESC, Y9LogSearchConsts.LOGIN_TIME));
		Page<Y9logUserLoginInfo> userLoginInfoPage = y9logUserLoginInfoRepository
			.findByTenantIdAndManagerLevel(tenantId, managerLevel, pageable);
	    return userLoginInfoPage;
	}
	
	@Override
	public Y9Page<Map<String, Object>> pageByUserHostIpAndSuccess(String userHostIp, String success,
			int page, int rows) {
        ...
        SearchRequest searchRequest = SearchRequest.of(s -> s
        	...
        	.query(q -> q
        		.bool(b -> b
        			.must(m -> m
        				.term(t -> t.field(Y9LogSearchConsts.USER_HOST_IP).value(userHostIp))
        			)
        			.must(m -> m
        				.term(t -> t.field(Y9LogSearchConsts.SUCCESS).value(success))
        			)
        		)
        	)
        	.aggregations("username-aggs", a -> a
        		.terms(t -> t.field(Y9LogSearchConsts.USER_NAME))
        		.aggregations("topHits", aggs -> aggs
        			.topHits(h -> h.size(1).explain(true)))
        	)
        );
        ...
        try {
			...
			for (int i = startIndex; i < endIndex; i++) {
				...
				Y9logUserLoginInfo y9logUserLoginInfo = bucket.aggregations().get("topHits")
	...
	@Override
	public Y9Page<Map<String, Object>> pageByUserHostIpAndSuccessAndUserNameLike(String userHostIp, 
			String success, String userName, int page, int rows) {
        int endIndex = page * rows;
        List<Map<String, Object>> strList = new ArrayList<>();
        ...
        SearchRequest searchRequest = SearchRequest.of(s -> s
        	...
        	.query(q -> q
        		.bool(b -> b
        			.must(m -> m.term(t -> t.field(Y9LogSearchConsts.USER_HOST_IP).value(userHostIp)))
	...
	public Y9Page<Y9logUserLoginInfo> pageByUserHostIpAndUserIdAndTenantIdAndLoginTime(String hostIp, String personId, 
        ...
        if (StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime)) {
            ...
            try {
                criteria.and(Y9LogSearchConsts.LOGIN_TIME)
                	.between(sdf.parse(startTime).getTime(), sdf.parse(endTime).getTime());
            } catch (ParseException e) {
                LOGGER.warn(e.getMessage(), e);
            }
        }
