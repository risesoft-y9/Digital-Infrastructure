...
public class Y9CommonAppForPersonServiceImpl implements Y9CommonAppForPersonService {
	...
	public String getAppNamesByPersonId(String personId) {
		SearchRequest searchRequest = SearchRequest.of(s -> s
			.index(Y9ESIndexConst.CLICKED_APP_INDEX)
			.query(q -> q
				.bool(b -> b
					.must(m -> m.queryString(qs -> qs.fields("personId").query(Y9Util.escape(personId))))
					.must(m -> m.queryString(qs -> qs.fields(Y9LogSearchConsts.TENANT_ID).query(Y9Util.escape(Y9LoginUserHolder.getTenantId()))))
					)
				)
			.aggregations("by_appName", a -> a
				.terms(t -> t.field("appName").size(100000)))
		...
		try {
			...
			elasticsearchClient.search(searchRequest, Void.class)
				.aggregations().get("by_appName").sterms().buckets().array()
					.forEach(bucket -> {
	...
	public String getAppNamesFromLog(String personId) {
        ...
        SearchRequest searchRequest = SearchRequest.of(s -> s
        	...
        	.query(q -> q
        		.bool(b -> b
        			.must(m -> m.queryString(qs -> qs.fields(Y9LogSearchConsts.MODULAR_NAME).query(APP_MODULARNAME)))
        			.must(m -> m.queryString(qs -> qs.fields("userId").query(personId)))
        			.must(m -> m.queryString(qs -> qs.fields(Y9LogSearchConsts.TENANT_ID).query(Y9LoginUserHolder.getTenantId())))
        			.must(m -> m.range(r -> r.field(Y9LogSearchConsts.LOG_TIME).from(String.valueOf(startTime)).to(String.valueOf(endTime))))
        			)
        		)
        	.aggregations("by_methodName", a -> a
        		.terms(t -> t.field("methodName").size(100000)))
        	.trackTotalHits(h -> h.enabled(true))
        	);
        
        try {
        	List<String> list = new ArrayList<>();
			elasticsearchClient.search(searchRequest, Void.class).aggregations()
				.get("by_methodName").sterms().buckets().array()
					.forEach(bucket -> {
						String appName = bucket.key().toString();
						list.add(appName);
					});
