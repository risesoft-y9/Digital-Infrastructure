...
public class Y9logAccessLogServiceImpl implements Y9logAccessLogService {
    ...
    public Map<String, Object> getAppClickCount(String orgId, String orgType, String tenantId, String startDay,
        ...
        build.must(m -> m.queryString(qs -> qs.fields(Y9LogSearchConsts.MODULAR_NAME)
            .query("net.risesoft.controller.admin.WebsiteController.saveAppCheckCount")));
        ...
        SearchRequest request = SearchRequest.of(s -> s.index(Arrays.asList(createIndexNames(startDay, endDay)))
            .query(build.build()._toQuery()).aggregations("by_appName", a -> a.terms(t -> t
                .field(Y9LogSearchConsts.METHOD_NAME).size(10000).order(new NamedValue<>("_count", SortOrder.Desc)))));
