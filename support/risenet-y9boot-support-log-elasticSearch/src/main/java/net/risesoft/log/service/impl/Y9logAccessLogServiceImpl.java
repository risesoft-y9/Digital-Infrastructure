...
public class Y9logAccessLogServiceImpl implements Y9logAccessLogService {
    ...
    @Override
    public Page<Y9logAccessLog> pageElapsedTimeByCondition(LogInfoModel searchDto, String startDay, String endDay, String sTime, String lTime, int rows, int page) throws ParseException {
        String tenantId = Y9LoginUserHolder.getTenantId();
        BoolQueryBuilder query = QueryBuilders.boolQuery();
        query.must(QueryBuilders.existsQuery(Y9LogSearchConsts.USER_NAME));
        if (StringUtils.isNotBlank(startDay) && StringUtils.isNotBlank(endDay)) {
