...
public class OidcLogoutEndpointController extends BaseOidcController {
    ...
    public OidcLogoutEndpointController(
     ...
     */
    @GetMapping({'/' + OidcConstants.BASE_OIDC_URL + '/' + OidcConstants.LOGOUT_URL,
        '/' + OidcConstants.BASE_OIDC_URL + "/logout", "/**/" + OidcConstants.LOGOUT_URL})
    public ResponseEntity handleRequestInternal(
        ...
        @RequestParam(value = "id_token_hint", required = false) final String idToken, final HttpServletRequest request,
        final HttpServletResponse response) throws Exception {

        val webContext = new JEEContext(request, response);
        if (!getConfigurationContext().getIssuerService().validateIssuer(webContext, OidcConstants.LOGOUT_URL)) {
            val body = OAuth20Utils
                .toJson(OAuth20Utils.getErrorResponseBody(OAuth20Constants.INVALID_REQUEST, "Invalid issuer"));
        ...
        if (StringUtils.isNotBlank(idToken)) {
            ...
            val registeredService =
                OAuth20Utils.getRegisteredOAuthServiceByClientId(configContext.getServicesManager(), clientId);
            ...
            if (StringUtils.isNotBlank(postLogoutRedirectUrl) && registeredService.getMatchingStrategy() != null) {
                ...
                if (matchResult) {
                    ...
                    return new ResponseEntity<>(
                        executeLogoutRedirect(Optional.ofNullable(StringUtils.trimToNull(state)),
     ...
     * @throws Exception the exception
     */
    protected HttpStatus executeLogoutRedirect(final Optional<String> state, final Optional<String> redirectUrl,
        final Optional<String> clientId, final HttpServletRequest request, final HttpServletResponse response)
        throws Exception {
        redirectUrl.ifPresent(url -> {
            ...
            request.setAttribute("service", url); // y9 add
        });
