...
public class LoginController {
    ...
    public LoginController(CentralAuthenticationService centralAuthenticationService, @Qualifier("ticketGrantingTicketCookieGenerator") CasCookieBuilder ticketGrantingTicketCookieGenerator,
        @Qualifier("defaultAuthenticationSystemSupport") AuthenticationSystemSupport authenticationSystemSupport,
        @Qualifier("webApplicationServiceFactory") ServiceFactory webApplicationServiceFactory, @Qualifier("logoutManager") LogoutManager logoutManager,
    ...
    public final ResponseEntity<Map<String, Object>> logon(RememberMeUsernamePasswordCredential credential, final HttpServletRequest request, final HttpServletResponse response) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(logoutTgtId)) {
                ...
                if (ticket != null) {
                    logoutManager
                        .performLogout(SingleLogoutExecutionRequest.builder().ticketGrantingTicket(ticket).httpServletRequest(Optional.of(request)).httpServletResponse(Optional.of(response)).build());
                }
            }
            ...
            final Ticket serviceTicket = this.centralAuthenticationService.grantServiceTicket(tgtId, service, authenticationResult);
