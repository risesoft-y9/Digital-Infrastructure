    ...
    public void saveScanResult(String uuid, String userId, HttpServletResponse response) {
        try {
            if (StringUtils.isEmpty(uuid) || StringUtils.isEmpty(userId)) {
                Y9Util.renderJson(response, Y9JacksonUtil.writeValueAsString(Y9Result.failure(400, "请求参数有问题：uuid或userId不能为空")));
                return;
            }
            ...
            if (null != obj) {
                if (obj.contains("1$")) {
                    Y9Util.renderJson(response, Y9JacksonUtil.writeValueAsString(Y9Result.failure(419, "二维码已过期：已被扫描。")));
                } else {
                ...
                Y9Util.renderJson(response, Y9JacksonUtil.writeValueAsString(Y9Result.failure(419, "二维码已过期：uuid已过期自动清除。")));
            }
        } catch (Exception e) {
            LOGGER.warn(e.getMessage(), e);
        }
