 ...
 */
@Entity
@Table(name = "Y9_LOG_CLICKED_APP", comment = "应用点击信息表")
@NoArgsConstructor
@Data
public class Y9ClickedApp implements Serializable {
    ...
    /** 应用ID */
    @Column(name = "APP_ID", length = 38, nullable = false, comment = "应用ID")
    private String appId;

    /** 应用名称 */
    @Column(name = "APP_NAME", length = 255, comment = "应用名称")
    private String appName;

    /** 保存日期 */
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd HH:mm:ss", timezone = "GMT+8")
    @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    @Column(name = "SAVE_DATE", comment = "保存日期")
