	...
	elInputCheck: {
		...
		mounted(el,binding,vnode,prevnode){
			...
			input[0].addEventListener('blur',(e) =>{
				...
				if(requiredCondition || RegularMsg){//如果为必填，或者正则校验不通过
					...
					if(spanTag.length > 0){//如果有span标签则隐藏
			...
			input[0].addEventListener('blur',(e) =>{
				...
				if(requiredCondition || RegularMsg){//如果为必填，或者正则校验不通过
					...
					watch(
						() => useSettingStore().getWebLanguage, //监听语言变化，配置对应的语言包
						(newLang) => { 
							document.getElementById('custom-validator-text').innerHTML=requiredCondition ? t(message) : t(RegularMsg) ;//显示错误信息在span标签中
						}, 
						{ 
							immediate:true
						} 
					...
					if(document.getElementById('custom-validator-text')){//如果有span标签则隐藏
