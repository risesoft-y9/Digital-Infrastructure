	   ...
	   let result = Object.assign({},...param)
	    ...
	    for(let objItem of param){
	        ...
	        for(let [key,val] of Object.entries(objItem)){
	            ...
	            if(Object.prototype.toString.call(result[key]) == '[object Object]' && Object.prototype.toString.call(val) == '[object Object]'){//对象深度拷贝合并
	            
	                result[key] = $deepAssignObject(result[key],val)
	
	            }else if(Object.prototype.toString.call(result[key]) == '[object Array]'&& Object.prototype.toString.call(val) == '[object Array]'){//数组深度拷贝
	...
	*/
	...
	if(replace){
		...
		Object.keys(replace).forEach(replaceKey => {
			...
			obj1[replaceKey] = obj2[replace[replaceKey]] !== undefined ? obj2[replace[replaceKey]] : replace[replaceKey];
...
export function $formatTime(param){
	   ...
	   if(param){ // 处理传过来的参数
	   	...
	   	if(param instanceof Date){ // 是日期对象
	   		...
	   		if(timestamp.toString().length === 10 || timestamp.toString().length === 9 || timestamp.toString().length === 8){ // 精确到'秒'的时间戳
...
export function $objEqual(obj1, obj2){
	...
	let isObj = object => { // 判断是否为对象类型
		 return object && typeof(object) === 'object' && Object.prototype.toString.call(object).toLowerCase() === "[object object]";
	};
	
	let isArray = object => { // 判断是否为数组
	    return object && typeof(object) === 'object' && object.constructor === Array;
	};
	
	let getLength = object => { // 获取长度
	    let count = 0;
	    for(let i in object) count++;
	    return count;
	}
	
	let compareObj = (obj1, obj2, flag) => { // 比较对象是否相同
	
	    for (let key in obj1) {
			
	        if (!flag) break;
			
	        if (!obj2.hasOwnProperty(key)) { //判断obj2是否存在obj1的属性
	            flag = false;
	            break;
	        }
			
			if(isObj(obj1[key])){ // 是对象时
				
				if (!isObj(obj2[key])) { //A当前value为对象，B如果不是，返回false
				    flag = false;
				    break;
				}
			...
			}else if(isArray(obj1[key])){ // 是数组时
				
				if (!isArray(obj2[key])) { //A当前value为数组，B如果不是，返回false
				    flag = false;
				    break;
				}
