<template>
    <el-dropdown  @command="onMenuClick"  :hide-on-click="true" class="user-el-dropdown">
        <div class="name" @click="e => e.preventDefault()">
            ...
            <el-avatar v-if="settingStore.device === 'mobile'" 
            :style=" { 'font-size': fontSizeObj.baseFontSize, 'background-color': 'var(--el-color-primary)', 'margin-top': '8px' }"
            :src="userInfo.avator?userInfo.avator:''"> {{userInfo.loginName}} </el-avatar>
        </div>
        <template #dropdown> 
            <el-dropdown-menu>
