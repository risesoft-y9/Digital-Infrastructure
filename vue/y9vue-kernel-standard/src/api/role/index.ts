...
export const roleTreeList = async (params) => {
  return await roleRequest({
    ...
    url: '/api/rest/role/listByParentId',
    method: 'get',
    cType: false,
    params
  });
...
export const getPublicRoleTree = async () => {
  return await roleRequest({
    ...
    url: `/api/rest/publicRole/treeRoot`,
    method: 'get',
    cType: false,
  })
}
...
// 根据资源ID，角色,和code模糊查询 
export const searchOperations = async (roleId, resourceName, operationType) => {
  const data = qs.stringify({
    roleId,
    resourceName,
    operationType
  ...
  return await roleRequest({
    url: '/api/rest/authorization/searchOperations',
    method: 'POST',
    cType: false,
    data
  })
