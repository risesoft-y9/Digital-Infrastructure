<template>
	<y9Card :title="`${$t('用户组列表')}${currInfo.name ? ' - ' + currInfo.name : ''}`">
		...
		<y9Table ref="groupTableRef" v-model:selectedVal="groupSelectedData" :config="groupListTableConfig" @on-current-change="onCurrentChange"></y9Table>
	</y9Card>
	<y9Dialog v-model:config="dialogConfig">
		<selectTree ref="selectTreeRef" :treeApiObj="treeApiObj" :selectField="[{fieldName:'orgType',value:['Group']},{fieldName:'disabled',value:false,}]"></selectTree>
	</y9Dialog>
...
<script lang="ts" setup>
	...
	const data = reactive({
		...
		groupListTableConfig: {//用户组列表表格配置
			columns: [
				{
					type:"radio",
					width:80,
		...
		dialogConfig: {
			...
			onOk: (newConfig) => {
				...
				return new Promise(async (resolve, reject) => {
					...
					await addGroups(currInfo.value.id,orgBaseIds.toString()).then(result => {
						ElNotification({
							title: result.success ? t('成功') : t('失败'),
							message: result.msg,
							type: result.success ? 'success' : 'error',
