<template>
	<y9Card :title="`${$t('设置部门领导')}${currInfo.name ? ' - ' + currInfo.name : ''}`">
		...
		<y9Dialog v-model:config="dialogConfig">
			<selectTree ref="selectTreeRef" :treeApiObj="typeName== 'org' ? treeApiObj : positionTreeApiObj" 
			:selectField="typeName== 'org' ? [{fieldName: 'orgType',value: ['Person']},{fieldName: 'disabled',value: false}] :
			[{fieldName: 'orgType',value: ['Position']},{fieldName: 'disabled',value: false}]"></selectTree>
		</y9Dialog>
	</y9Card>
...
<script lang="ts" setup>
	...
	import {setDeptLeaders,getDeptLeaders,removeLeader} from '@/api/dept/index';
	...
	const data = reactive({
		...
		setLeaderTableConfig: {//设置部门领导表格配置
			columns: [
				{
					title: computed(() => t("姓名")),
					key: "name",
		...
		dialogConfig: {
			...
			onOk: (newConfig) => {
				return new Promise(async (resolve, reject) => {
					...
					await setDeptLeaders(currInfo.value.id,orgBaseIds.toString()).then(result => {
						ElNotification({
							title: result.success ? t('成功') : t('失败'),
							message: result.msg,
							type: result.success ? 'success' : 'error',
