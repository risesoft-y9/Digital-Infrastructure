<template>
	<y9Card :title="`${$t('基本信息')}${ currInfo.name ? ' - ' + currInfo.name : '' }`">
		...
		<div class="oth-btns" v-show="currInfo.haveEditAuth">
			<div v-if="isEditState">
				<el-button type="primary" :loading="saveFormBtnLoading" 
				:style="{ fontSize: fontSizeObj.baseFontSize }" :size="fontSizeObj.buttonSize"  
				@click="onActions('save')" class="global-btn-main">
			...
			<div v-else style="display: flex;justify-content: space-between;text-align: right;">
				...
				<div style="margin-left: 10px;">
					<el-button v-show="showBtn('addDepartment')" type="primary"
					:style="{ fontSize: fontSizeObj.baseFontSize }" :size="fontSizeObj.buttonSize" 
					class="global-btn-main" @click="onActions('addDepartment',`${currInfo.orgType == 'Organization' ? '新增部门' : '新增子部门'}`)">
						<i class="ri-add-line"></i>
						<span>{{currInfo.orgType == 'Organization' ? $t('部门') : $t('子部门')}}</span>
					</el-button>
					<el-button v-show="showBtn('addPosition')" type="primary" 
					:style="{ fontSize: fontSizeObj.baseFontSize }" :size="fontSizeObj.buttonSize" 
					class="global-btn-main" @click="onActions('addPosition','新增岗位')">
						<i class="ri-add-line"></i>
						<span>{{ $t('岗位') }}</span>
					</el-button>
					<el-button v-show="showBtn('move')" :size="fontSizeObj.buttonSize" 
					:style="{ fontSize: fontSizeObj.baseFontSize }"  @click="onActions('move','移动')" class="global-btn-second">
						<i class="ri-route-line"></i>
						<span>{{ $t('移动') }}</span>
					</el-button>
					<el-button v-show="showBtn('sort')" :size="fontSizeObj.buttonSize"
					:style="{ fontSize: fontSizeObj.baseFontSize }"  @click="onActions('sort','综合排序')" class="global-btn-second">
					...
					<el-button v-show="showBtn('extendAttr')" :size="fontSizeObj.buttonSize" :style="{ fontSize: fontSizeObj.baseFontSize }"
					 @click="onActions('extendAttr','扩展属性')" class="global-btn-second">
						<i class="ri-external-link-line"></i>
						<span>{{ $t('扩展属性') }}</span>
					</el-button>
					<el-button v-show="showBtn('sync')" :size="fontSizeObj.buttonSize" 
					:style="{ fontSize: fontSizeObj.baseFontSize }" @click="onActions('sync','数据同步')" class="global-btn-second">
		...
		<div>
			<orgForm v-if="currInfo.orgType == 'Organization'" ref="orgFormRef" :currInfo="currInfo" :isEditState="isEditState"></orgForm>
			<departmentForm v-if="currInfo.orgType == 'Department'" ref="deptFormRef" :currInfo="currInfo" :isEditState="isEditState"></departmentForm>
			<positionForm v-if="currInfo.orgType == 'Position'" ref="positionFormRef"  :currInfo="currInfo" :isEditState="isEditState"></positionForm>
		</div>
	...
	<y9Dialog v-model:config="dialogConfig">
		<departmentForm v-if="dialogConfig.type == 'addDepartment'" ref="addDeptFormRef" :currInfo="currInfo" :isEditState="true" isAdd></departmentForm>
		<positionForm v-if="dialogConfig.type == 'addPosition'" ref="addPositionFormRef" :currInfo="currInfo" :isEditState="true" isAdd></positionForm>
		<extendAttr v-if="dialogConfig.type == 'extendAttr'" :currInfo="currInfo" :handAssginNode="handAssginNode"></extendAttr>
		...
		></selectTree>
		<treeSort v-if="dialogConfig.type == 'sort'" :currInfo="currInfo" :apiRequest="getOrderDepts" :apiParams="currInfo.id" :columns="dialogConfig.columns" type="Position" ref="sortRef"></treeSort>
	</y9Dialog>
...
<script lang="ts" setup>
	...
	import {positionSaveOrUpdate,movePosition,savePositionExtendProperties} from '@/api/position/index';
	...
	const data = reactive({
			...
			onOk: (newConfig) => {
				return new Promise(async (resolve, reject) => {
					...
					if(newConfig.value.type == 'addDepartment'){
						...
						await deptSaveOrUpdate(formData).then(async res => {
							result = res;
						...
						await positionSaveOrUpdate(formData).then(async res => {
							result = res;
							//更新当前节点的children信息到树数据当中
							await props.handAssginNode({},currInfo.value.id,currInfo.value.id)//手动更新节点到tree
						}).catch(() => {});
						...
						if(selectIds.length == 0){
							ElNotification({title: '失败',message: '请选择部门',type: 'error',duration: 2000,offset: 80});
							reject();
							return;
						}
						const  targetId  = selectIds[0];
						if(props.currTreeNodeInfo.orgType == 'Department'){
							await moveDept(props.currTreeNodeInfo.id,targetId).then(res => {
								result = res;
							}).catch(()=> {});
						}else if(props.currTreeNodeInfo.orgType == 'Position'){
							await movePosition(props.currTreeNodeInfo.id,targetId).then(res => {
								result = res;
							}).catch(()=> {});
						}
						...
						await saveOrder(props.currTreeNodeInfo.id,ids.toString()).then(async res => {
							result = res;
							if (res.success) {
								//更新当前节点的children信息到树数据当中
								await props.handAssginNode({},currInfo.value.id,currInfo.value.id)//手动更新节点到tree
                        ...
                        await sync(params).then(res => {result = res}).catch(() => {});
                    } 
					ElNotification({
		...
		deptTreeApiObj:{//tree接口对象
			...
			childLevel: {
			    api: async () => {
					...
					data.forEach(item => {//禁止选择移动到自己本身,也禁止选择移动到自身的子节点
		...
		selectTreeRef:'',
		orgFormRef:"",//编辑机构表单实例
		deptFormRef:"",//编辑部门表单实例
		positionFormRef:"",//编辑岗位表单机构
		addDeptFormRef:"",//新增部门表单实例
		addPositionFormRef:"",//新增部门表单实例
		syncRef: "", // 同步表单实例
		sortRef:''
	})
	...
	async function onActions(type,title?) {
		...
		} else if (type == 'addDepartment' || type == 'addPosition' || type == 'extendAttr' || type == 'move' || type == 'sync' || type == 'sort') {//点击按钮显示弹窗
