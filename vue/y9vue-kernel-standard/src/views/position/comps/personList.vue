								...
								ElMessageBox.confirm(
									...
									if(result.success){
										...
										if(personListTableConfig.value.tableData.length > 0){
											name = `${currInfo.value.jobName}  (${personListTableConfig.value.tableData.map(item => item.name).join('，')})`;
										}
		...
		dialogConfig: {
			...
			onOk: (newConfig) => {
				...
				return new Promise(async (resolve, reject) => {
					...
					await addPersons(currInfo.value.id,orgBaseIds.toString()).then(result => {
						if(result.success){
