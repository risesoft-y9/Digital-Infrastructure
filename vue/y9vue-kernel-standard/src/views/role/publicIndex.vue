                            ...
                            <div class="basic-btns" v-if="managerLevel === 1">
                                <div class="btn-top">
                                    ...
                                    <span v-if="currData.type !== 'role'">
                                        ...
                                        :size="fontSizeObj.buttonSize"
                                            :style="{ fontSize: fontSizeObj.baseFontSize }" @click="handlerAddRole">
                        ...
                        >
                            <template v-slot:filterBtnSlot>
                                <el-button class="global-btn-main" :size="fontSizeObj.buttonSize"
                                            :style="{ fontSize: fontSizeObj.baseFontSize }"
                                @click="initList" type="primary">
                                    <i class="ri-search-line"></i>
                                    {{ $t('搜索') }}
                                </el-button>
                                <div class="widthBtn">
                                    <el-button @click="handlerClick('positive')" :size="fontSizeObj.buttonSize"
                                            :style="{ fontSize: fontSizeObj.baseFontSize }"
                                    class="global-btn-second">
                                        <i class="ri-add-line"></i>
                                        {{ $t('正权限人员') }}
                                    </el-button>
                                    <el-button @click="handlerClick('negative')" :size="fontSizeObj.buttonSize"
                                            :style="{ fontSize: fontSizeObj.baseFontSize }"
                                    class="global-btn-second">
                                        <i class="ri-add-line"></i>
                                        {{ $t('负权限人员') }}
                    ...
                    >
                        ...
                        >
                            <template v-slot:filterBtnSlot>
                                ...
                                <div class="widthBtn">
                                    <el-button @click="handlerClickResource"
                                    :size="fontSizeObj.buttonSize"
                                            :style="{ fontSize: fontSizeObj.baseFontSize }"
                                    class="global-btn-second">
                                        <i class="ri-add-line"></i>
                                ...
                                <el-button @click="handlerDeleteClick('resource')" :size="fontSizeObj.buttonSize"
                                            :style="{ fontSize: fontSizeObj.baseFontSize }"
                                class="global-btn-second">
                                    <i class="ri-close-line"></i>
                                    {{ $t('删除') }}
    ...
    let sortDialogConfig = ref({
        ...
        onOk: (newConfig) => {
            return new Promise(async (resolve, reject) => {
                ...
                await saveOrder(ids.toString()).then(res => {result = res}).catch(() => {});
            ...
            return new Promise(async (resolve, reject) => {
               ...
               if(ids.length === 0){
                ...
                if (negativeConfigDialog.value.title ==  computed(() => t("添加正权限人员")).value) {
                    await addOrgUnits(currData.value.id, ids.join(','), false).then(res => {result = res}).catch(() => {});
                } else if (negativeConfigDialog.value.title == computed(() => t("添加负权限人员")).value) {
                    await addOrgUnits(currData.value.id, ids.join(','), true).then(res => {result = res}).catch(() => {});
                }
                ElNotification({
