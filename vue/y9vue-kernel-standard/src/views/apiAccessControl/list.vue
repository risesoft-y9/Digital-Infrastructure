<template>
    ...
    >
        ...
        <template #ipList="{ row, column, index }">
            <el-input v-if="currEditId === index" v-model="editForm.value" :size="fontSizeObj.buttonSize"></el-input>
...
<script lang="ts" setup>
    ...
    import { changeEnabled, list, remove, saveOrder, saveUpdate, search } from '@/api/apiAccessControl/index';
...
width: 200
            ...
            {
                title: computed(() => t('IP地址')),
                key: 'value',
                slot: 'ipList',
                props: {
                    placeholder:
                        '多个用英文逗号分割，支持具体 IP、IP 网段格式，例子：192.168.1.1,192.168.1.0/24,192.168.1.1-100,192.168.1.*'
                },
                            ...
                            'div',
                            {
                                ...
                                onClick: async () => {
                                    ...
                                    if (!editForm.value.value) {
                                        ...
                                        apiAccessControlTableConfig.value.loading = true;
                                        const res = await saveUpdate(editForm.value);
                                        if (res.code == 0 && res.success) {
                                            if (!editForm.value.id) {
                                                //如果是新增就手动删除
                                                apiAccessControlTableConfig.value.tableData.splice(params.$index, 1);
    ...
    async function getAccessControlList() {
        apiAccessControlTableConfig.value.loading = true;
        const res = await list({ type: props.type });
        if (res.code == 0 && res.success) {
            apiAccessControlTableConfig.value.tableData = res.data;
        }
        apiAccessControlTableConfig.value.loading = false;
