...
public class DefaultCasCookieValueManager extends EncryptedCookieValueManager {
    ...
    protected String obtainValueFromCompoundCookie(final String value, final HttpServletRequest request) {
        ...
        val clientInfo = ClientInfoHolder.getClientInfo();
        if (clientInfo == null) {
            val message = "Unable to match required remote address " + cookieIpAddress
        ...
        if (!cookieIpAddress.equals(clientIpAddress)) {
            if (StringUtils.isBlank(cookieProperties.getAllowedIpAddressesPattern())
                ...
                val message =
                    "Invalid cookie. Required remote address " + cookieIpAddress + " does not match " + clientIpAddress;
                LOGGER.warn(message);
                throw new InvalidCookieException(message);
            }
        ...
        if (!cookieUserAgent.equals(agent)) {
            if (cookieUserAgent.contains("Chrome") && agent.contains("Chrome") && !cookieUserAgent
                .substring(0, cookieUserAgent.indexOf("Chrome")).equals(agent.substring(0, agent.indexOf("Chrome")))) { // y9
                                                                                                                        // edit
                val message =
                    String.format("Invalid cookie. Required user-agent %s does not match %s", cookieUserAgent, agent);
                LOGGER.warn(message);
                throw new InvalidCookieException(message);
            } // y9 edit
        }
