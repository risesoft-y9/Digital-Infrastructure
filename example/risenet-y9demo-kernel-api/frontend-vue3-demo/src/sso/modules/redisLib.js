 ...
 * @LastEditTime: 2022-01-18 18:06:13
...
const redisRequest = axios.create({
    baseURL: y9_storage.getObjectItem('sso', "VUE_APP_SERVER_REDIS"),
...
// 请求拦截器 
redisRequest.interceptors.request.use(
    config => {
...
// 响应拦截器
redisRequest.interceptors.response.use(
    response => {
        let res;
        if (response.data) {
            res = response.data;
