...
(
    ...
    EMAIL          varchar(255) comment '电子邮箱',
    INNER_ROLE     varchar(255) comment '内部角色',
    LOGIN_NAME     varchar(255)                      not null comment '登录名',
...
create table Y9_ORG_PERSONS_JOBS
(
    ID           integer           not null auto_increment comment '主键',
    CREATE_TIME  datetime(6) comment '创建时间',
    UPDATE_TIME  datetime(6) comment '更新时间',
    JOB_ID       varchar(38)       not null comment '职位ID',
    JOB_ORDER    integer default 0 not null comment '职位排序号',
    PERSON_ID    varchar(38)       not null comment '人员ID',
    PERSON_ORDER integer default 0 not null comment '人员排序号',
    primary key (ID)
) comment='人员职位关联表' engine=InnoDB;
...
alter table Y9_ORG_PERSONS_JOBS
    add constraint UKkbgp701hd84qgdmlijf9s0ogi unique (JOB_ID, PERSON_ID);
alter table Y9_ORG_PERSONS_POSITIONS
    add constraint UKrty6axgf79bgsw4vxtki54i3c unique (POSITION_ID, PERSON_ID);
create index IDX4avacoftvorjgda8cxoi95w5m on Y9_ORG_PERSONS_RESOURCES (PERSON_ID, RESOURCE_ID);
...
create index IDX3jaa0vbyrbedj5bku0nut0i1j on Y9_ORG_POSITIONS_RESOURCES (POSITION_ID, RESOURCE_ID);
